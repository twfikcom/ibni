...
import { Utensils, IceCream, Sandwich, ShoppingBasket, X, Trash2, Send, Plus, Minus, Truck, Loader2, Star, Sparkles, MapPin, Phone, User, AlertCircle, MessageSquare, Facebook, MessageCircle, Smartphone } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleFinalSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setShowSuccess(true);
        // Refresh or close after some time, unless installing
        if (!deferredPrompt) {
            setTimeout(() => {
                location.reload();
            }, 8000);
        ...
        {!loading && (
          ...
          >
            ...
            <main className="max-w-7xl mx-auto px-4 pt-4 relative z-10 pb-32">
              <Hero onInstallClick={deferredPrompt ? handleInstallClick : undefined} />
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[5000] bg-black flex flex-col items-center justify-center p-8 text-center">
                  ...
                  {deferredPrompt && (
                    <motion.button 
                      ...
                      className="bg-white/10 border border-[#FAB520]/30 text-[#FAB520] px-8 py-4 rounded-3xl flex items-center gap-3 font-bold text-lg"
                    >
                      <Smartphone className="w-6 h-6" />
                      <span>تثبيت تطبيق يا عم للمرات الجاية!</span>
                    </motion.button>
                  )}
                  
                  {!deferredPrompt && (
                    <button onClick={() => location.reload()} className="text-gray-500 mt-4 underline">العودة للرئيسية</button>
                  )}
                </motion.div>
              )}
            </AnimatePresence>
